...
extension ProfileInfoViewController:UIImagePickerControllerDelegate{
    ...
    @objc private func loggIn(){
        ...
        if nameTextField.text != "" {
            ...
            if let image = self.profileImageButton.imageView?.image?.pngData(){
                profileImage = image
            }
            else {
                profileImage =  UIImage(systemName: "person.fill")?.pngData()
                
            }
            
            let fileName = userInfo.profilePictureFileName
            StorageManager.shared.uploadProfilePicture(with: profileImage, fileName: fileName) { result in
                switch result {
